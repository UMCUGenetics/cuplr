...
model_dir <- paste0(base_dir,'/CUPs_classifier/processed/cuplr/cuplr/models/0.18a_newLabels/')
...
df_features <- read.delim(
   paste0(base_dir,'/CUPs_classifier/processed/features/_all/04_kidneySplit/all_features.txt.gz'),
   check.names=F
)
...
sample_whitelist <- metadata[metadata$in_train_set,'sample_id']
#sample_whitelist <- subset(metadata, training_group>=1, sample_id, drop=T)

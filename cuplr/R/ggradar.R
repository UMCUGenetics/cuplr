...
ggradar <- function(
   ...
   df <- subset(axis$label, axis$label$x < (-x.centre.range))
   ...
   df <- subset(axis$label, abs(axis$label$x) <= x.centre.range)
   base <- base + geom_text(
      data = df,

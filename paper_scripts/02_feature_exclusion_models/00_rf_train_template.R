...
if(F){
   wd <- paste0(base_dir,'/CUPs_classifier/processed/cuplr/cuplr/models/0.21a_DR104-update5_pcawgSoftFilt2/')
...
if(grepl('[.]rds$',training_data_path)){
   training_data <- readRDS(training_data_path)
} else {
   training_data <- read.delim(training_data_path, check.names=F)
}
...
alternative[grep('^chrom_arm.loss',names(alternative))] <- 'less'
alternative[names(alternative)=='sv.load'] <- 'two.sided'
alternative[names(alternative)=='gender.gender'] <- 'two.sided'
alternative[names(alternative)=='genome.ploidy'] <- 'two.sided'
alternative[names(alternative)=='genome.diploid_proportion'] <- 'two.sided'
...
if(!is.null(test)){
   ...
   test_set_report <- predict.randomForestEnsemble(
      ...
      type='report',
      filter.probs.by.gender=('gender.gender' %in% feature_names),
      verbose=T
   )

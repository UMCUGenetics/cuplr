...
################################################################################
univarFeatSel <- function(x, y, method='median', max.pvalue=0.01, verbose=T){
   #df=train
   #x <- df[,colnames(df)!=colname.response]
   #y <- as.factor(df[,colname.response])
   ...
   if(method=='median'){
      ...
      whitelistByPairwiseTest <- function(m_pos, m_neg){
         ...
         p_values <- unlist(lapply(1:ncol(m_pos), function(i){
            #i=1
            wilcox.test(m_pos[,i], m_neg[,i], alternative='greater')$p.value
         }))
         names(p_values) <- colnames(m_pos)
         
         names(p_values)[ p_values < max.pvalue ]
         
         # q_values <- p.adjust(p_values, method='bonferroni')
         # names(q_values) <- colnames(m_pos)
      }
...
# rmUselessFeatures <- function(x, y, verbose=T){
#    #df=train
#    #x <- df[,colnames(df)!=colname.response]
#    #y <- as.factor(df[,colname.response])
#    
...
#    features <- colnames(x)
#    
#    m <- do.call(rbind, lapply(1:length(classes), function(i){
#       #i=1
#       if(verbose){
#          message('Processing [',i,'/',length(classes),']: ',classes[i])
#       }
#       
#       pos_samples <- as.matrix(x_split[[i]])
#       neg_samples <- as.matrix(do.call(rbind, unname(x_split[-i])))
#       
#       med_pos <- apply(pos_samples,2,median)
#       med_neg <- apply(neg_samples,2,median)
#       
#       med_pos - med_neg
#    }))
#    dimnames(m) <- list(classes, features)
#    
#    feature_blacklist <- features[ !apply(m,2,function(i){ any(i > 0) }) ]
#    
#    x[,!(colnames(x) %in% feature_blacklist)]
# }

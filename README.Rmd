...
(ii) [PURPLE](https://github.com/hartwigmedical/hmftools/tree/master/purple) output, and 
(iii) [LINX](https://github.com/hartwigmedical/hmftools/tree/master/sv-linx) output. Below is a 
...
and the Pan-Cancer Analysis of Whole Genomes (PCAWG) consortium. Performance was assessed by 15-fold
cross-validation, as well as on a holdout validation set of ~680 samples.

For details on performance and important features used by CUPLR, please see the PDFs at `doc/perf/`.
...
dependencies, see the DESCRIPTION files.
...
* `nmf`: non-negative matrix factorization for generating regional mutational density signatures
...
Then load the `featureExtractor`, `cuplr`, and `mutSigExtractor` packages. The remaining CUPLR packages (`statsExtra` and `nmf`)
are only needed for training CUPLR.
```{r eval=FALSE, error=FALSE}
...
The scores outputted by a random forest need to be calibrated to yield true probabilities. A well 
calibrated classifier should classify the samples such that among the samples to which it gave a 
score close to e.g. 0.8, approximately 80% actually belong to the positive class. For more info, see 
the [scikit-learn documentation](https://scikit-learn.org/stable/modules/calibration.html)
...
Using `patientReport()` we can plot the class probabilities and feature contributions per class.
This is primarily useful for clinical reporting.
...
if(F){
   png(paste0(cuplr_dir,'/doc/supp/patient_report_example.png'), 1000, 700, res=150)
...
DO36107 is a pilocytic astrocytoma cancer sample and we can see that this sample was correctly 
predicted as such. The prediction is supported by this sample having a KIAA1549-BRAF fusion, a
